...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      // 加载浮层:暂停期间的视觉提示;层级低于按钮(60),保证加载中按钮仍可点击取消
  ...
  function injectCaptureButton() {
    ...
    if (existing) {
      if (controls) {
        existing.classList.remove('transnotes-shorts-btn');
        // 顺序:配音按钮在左,「记录想法」紧随其后
        const dubBtn = controls.querySelector('.' + BTN_CLASS);
        if (dubBtn) dubBtn.after(existing);
        else if (existing.parentNode !== controls) controls.insertBefore(existing, controls.firstChild);
      } else if (isShortsPage() && player) {
        existing.classList.add('transnotes-shorts-btn');
        if (existing.closest('#movie_player') !== player) player.appendChild(existing);
      }
  ...
  async function startDubbing() {
    ...
    if (cachedDoc && cachedDoc.cues && cachedDoc.cues.length) {
      // route 保持原样,缓存命中只作展示标记;旧版本曾把「(缓存)」拼进 route 并
      // 写回缓存,会逐次累积,读取时先清洗掉历史污染
      const baseRoute = (cachedDoc.route || '').replace(/(\(缓存\))+$/, '');
      sub = {
        cues: cachedDoc.cues,
        skipTranslate: typeof cachedDoc.skipTranslate === 'boolean'
          ? cachedDoc.skipTranslate
          : /中文字幕轨|自动翻译/.test(baseRoute),
        route: baseRoute,
        fromCache: true,
      };
