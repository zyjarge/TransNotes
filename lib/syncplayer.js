 ...
 *    视频减速(钳制 [0.65,1.25])与音频加速(钳制 [0.9,1.6])各承担一半误差,
 *    两者在窗口中点"会师",人耳几乎无感
 *    (此外合成侧还有预测式 TTS 提速,见 background.js predictChunkSpeed)
...
(function () {
  ...
  const VIDEO_RATE_MIN = 0.65;    // 视频调速钳制(相对用户倍速的系数;0.75→0.65 放宽吸超长余量)
  const VIDEO_RATE_MAX = 1.25;
  const AUDIO_RATE_MIN = 0.9;     // 音频调速钳制(相对用户倍速的系数)
  const AUDIO_RATE_MAX = 1.6;
  ...
  class SyncPlayer {
    ...
    _tick() {
      ...
      try {
        ...
        // 任何异常都不能杀死 tick 链
        console.error('[transnotes] 播放引擎异常(已自动恢复):', e);
      }
    ...
    async _switchTo(idx, t) {
      ...
      if (t - cue.start > MAX_LAG_SKIP) {
        if (d > 0) {
          ...
          if (d - offset < MIN_TAIL_SECONDS) {
            console.debug('[transnotes] 音频剩余过短,跳过句子:', idx);
            this._restoreVideoRate();
            return;
          }
        } else {
          // 时长未知且已严重迟到,无法计算对齐点,维持跳过策略
          console.debug('[transnotes] 音频迟到,跳过句子:', idx);
          this._exitBuffering();
          return;
        }
      }
      ...
      try {
        await el.play();
        console.debug('[transnotes] 播放句子:', idx);
      } catch (e) {
        // 播放被用户手势策略阻断等场景:静默等待下一句
        console.warn('[transnotes] 音频播放被阻止:', idx, e && e.message);
        this.audio = null;
        this._audioIndex = -1;
        this._restoreVideoRate();
      }
    ...
    _enterBuffering() {
      ...
      console.debug('[transnotes] 缓冲中:等待语音合成');
    }
