...
(function () {
   ...
   *  image 为 dataURL 时按 OpenAI 多模态格式携带图片(视觉模型) */
  async function chatOnce(ai, system, userContent, maxTokens, image) {
   ...
   * image 为 dataURL 时按多模态格式携带(仅视觉模型可用,调用方负责探测与降级)
   * @returns {Promise<string>} 模型输出文本
   */
  async function aiChat(ai, system, userContent, { maxTokens = 8192, image } = {}) {
    for (let attempt = 0; attempt < 3; attempt++) {
      const budget = Math.min(maxTokens * Math.pow(2, attempt), 32768);
      const r = await chatOnce(ai, system, userContent, budget, image);
  ...
  /** 概览缓存结构版本:
   *  v1=章节+关键引述;v2=概要+章节(概要 V2)
   *  v3=分段+合并(长视频适配:分段独立 AI + 末轮合并) */
  const OV_SCHEMA_V = 3;

  /** 字幕文档 → 带时间戳的转录文本(中文优先,超长截断)
   *
   * 长视频(>20k 字符字幕)拆为 N 段,每段前加段标头(起止时间+段号),
   * 段间用清晰的分隔行,便于 AI 识别分段并按段写章节。
   * 不再硬截断——AI 看到的总长 ≈ 实际字幕总长 + N 段头部。 */
  function buildTranscript(doc) {
    const cues = doc.cues || [];
    if (cues.length === 0) return '';
    const chunks = chunkByCharBudget(cues);
    if (chunks.length === 1) {
      // 短视频:无段头,保持原输出格式(用户体感不变)
      return chunks[0].lines.join('\n');
    }
    // 长视频:逐段渲染 + 段标头 + 全文末尾汇总
    const durLast = cues[cues.length - 1].end;
    const parts = [];
    for (let i = 0; i < chunks.length; i++) {
      const ch = chunks[i];
      parts.push(
        `===== 段 ${i + 1}/${chunks.length} (字幕时间 ${fmtTime(ch.startSec)} → ${fmtTime(ch.endSec)}) =====`
      );
      parts.push(ch.lines.join('\n'));
      parts.push(`===== 段 ${i + 1}/${chunks.length} 结束 =====`);
      parts.push('');
    }
    parts.push(`(全文共 ${chunks.length} 段,总时长 ${fmtTime(durLast)},${cues.length} 句)`);
    return parts.join('\n');
  }

  /**
   * 按字符预算把字幕切成多个块(块内尽量保持时间升序连续)。
   * 每块独立预算 ~TRANSCRIPT_CHAR_LIMIT;切点选在最近的句边界(中英标点)。
   * 返回 [{ startSec, endSec, lines:[...], cueCount }]
   */
  function chunkByCharBudget(cues, { budget = TRANSCRIPT_CHAR_LIMIT, minGapSec = 0.3 } = {}) {
    const lines = [];
    const chunks = [];
    let total = 0;
    let chunkStartSec = null;
    let chunkEndSec = null;       // 当前块最后一句的 end 时间(用于老块收尾)
    let chunkLines = [];
    let chunkTotal = 0;
    const flush = () => {
    ...
    for (const c of cues) {
      ...
      if (chunkStartSec === null) chunkStartSec = c.start;
      // 当前行追加后会超出本块预算,且本块已有内容 → 在句边界处切分
      if (chunkTotal + line.length > budget && chunkLines.length > 0) {
        // 找最近的句边界作为切点:从尾部向前回溯 12 行内
        const SENT_END = /[.!?;…。!?;]\s*$/;
        let cutIdx = chunkLines.length;
        for (let back = chunkLines.length - 1; back >= Math.max(0, chunkLines.length - 12); back--) {
          if (SENT_END.test(chunkLines[back])) { cutIdx = back + 1; break; }
        }
        if (cutIdx < chunkLines.length) {
          // chunkLines 拆成 [kept, tail]
          const kept = chunkLines.slice(0, cutIdx);
          const tail = chunkLines.slice(cutIdx);
          // 老块 endSec = kept 最后一句对应的 cue.end
          // 我们没单独存每行 end, 用 kept 末行解析出的 start + cue 经验时长(不精确但够用)
          // 更稳妥:从源 cues 反查 kept 末行的时间戳对应的 cue
          const keptLastMatch = kept[kept.length - 1] && kept[kept.length - 1].match(/^\[(\d+:\d+(?::\d+)?)\]/);
          const tailFirstMatch = tail[0] && tail[0].match(/^\[(\d+:\d+(?::\d+)?)\]/);
          const tailFirstSec = tailFirstMatch ? parseTimestampToSec(tailFirstMatch[1]) : c.start;
          // 找 kept 最后一行对应的 cue (startSec 最接近)
          const keptLastSec = keptLastMatch ? parseTimestampToSec(keptLastMatch[1]) : chunkStartSec;
          const keptLastCue = findCueNear(cues, keptLastSec);
          const keptEndSec = keptLastCue ? keptLastCue.end : keptLastSec;
          // 收尾老块
          chunks.push({
            startSec: chunkStartSec,
            endSec: keptEndSec,
          ...
          // 新块种子:从 tail 开始, startSec = tail 第一句 start
          chunkStartSec = tailFirstSec;
          chunkLines = tail;
          chunkEndSec = c.end; // 当前 c 还未入 tail, 但 c.end 是下一句的 end 也是新块的当前 end
          chunkTotal = tail.reduce((s, l) => s + l.length + 1, 0);
          // 注:当前行 c 还没 push;后续循环会处理
        } else {
          // 没有合适的句边界 → 直接整块结束
          flush();
          chunkStartSec = c.start;
          chunkEndSec = c.end;
          chunkTotal = 0;
        }
      }
      ...
      chunkEndSec = c.end;
      chunkTotal += line.length + 1;
      lines.push(line); total += line.length + 1;
    }
    flush();
    return chunks.length ? chunks : [{ startSec: 0, endSec: 0, lines, cueCount: lines.length }];
  }

  /** 在 cues 中找 start 最接近 targetSec 的 cue;二分(假设 cues 按 start 升序) */
  function findCueNear(cues, targetSec) {
    if (!Array.isArray(cues) || cues.length === 0) return null;
    let lo = 0, hi = cues.length - 1;
    while (lo < hi) {
      const mid = (lo + hi) >> 1;
      if (cues[mid].start < targetSec) lo = mid + 1;
      else hi = mid;
    }
    // lo 是第一个 start >= target 的;往前一个比较哪个更近
    if (lo > 0 && Math.abs(cues[lo - 1].start - targetSec) <= Math.abs(cues[lo].start - targetSec)) {
      return cues[lo - 1];
    }
    return cues[lo];
  }

  /** 把 [M:SS] / [H:MM:SS] / [MM:SS] 字符串转秒 */
  function parseTimestampToSec(s) {
   ...
   */
  const LEVEL_GUIDE = {
    concise: '章节控制在 3-5 个,每章摘要一句话',
    normal: '章节按内容自然划分,每章摘要 1-2 句',
    detailed: '章节尽量细分(8 个以上也可),每章摘要 2-3 句',
  };

  /**
   * 概览分段局部 prompt:章节只需要覆盖"本段"区间,
   * 时间戳范围 = 本段字幕时间范围
   */
  function chunkSystemPrompt(level, chunkMeta) {
    const guide = LEVEL_GUIDE[level] || LEVEL_GUIDE.normal;
    const range = `本段时间戳范围 ${fmtTime(chunkMeta.startSec)} → ${fmtTime(chunkMeta.endSec)}`;
    return (
      '你是我的学习助手。我会给你一段带时间戳的视频字幕(可能是中文或英文), ' +
      `这是长视频中的第 ${chunkMeta.idx}/${chunkMeta.total} 段,${range}。\n` +
      '请只针对本段输出一份中文局部概览(JSON):\n' +
      '1) chunkSummary:用 1-2 句话总结本段讲了什么\n' +
      `2) 章节:${guide};章节必须覆盖本段从 ${fmtTime(chunkMeta.startSec)} 到 ${fmtTime(chunkMeta.endSec)} 完整时间区间\n` +
      '3) 时间戳必须取自字幕行首的 [M:SS],不得编造,不得超过本段时间范围\n' +
      '只输出 JSON(不要 markdown 代码围栏),格式:\n' +
      '{"chunkSummary":"本段讲……","chapters":[{"title":"章节标题","timestamp":"0:00","timestampSeconds":0,"summary":"本节内容"}]}'
    );
  }

  /** 末轮合并 prompt:把所有局部段拼成全局概览 */
  function mergeSystemPrompt(level) {
    const guide = LEVEL_GUIDE[level] || LEVEL_GUIDE.normal;
    return (
      '你是我的学习助手。我会给你一段长视频被切分后每段的局部概览(每段含本段时间区间、章节列表、要点)。\n' +
      '请把这些局部章节合并成一份完整的中文全局概览,要求:\n' +
      '1) summary:以第三人称口吻连贯介绍整个视频讲了什么(3-5 句),关键信息处内嵌时间戳锚点 [M:SS](取自章节 timestamp)\n' +
      `2) 章节:全局 ${guide};章节必须覆盖视频从头到尾(每段都有代表章节),` +
      '最后一段必须有章节覆盖到视频后段,不要只集中在开头\n' +
      '3) 章节时间戳必须从局部章节的 timestamp 选取,不得编造,不得超过视频总时长\n' +
      '4) 局部章节覆盖足够时不要新增,只做去重/合并/排序;空白处用段交界时间戳补齐\n' +
      '只输出 JSON(不要 markdown 代码围栏),格式:\n' +
      '{"summary":"这个视频……[0:45]……","chapters":[{"title":"章节标题","timestamp":"0:00","timestampSeconds":0,"summary":"本节内容"}]}'
    );
  }
  ...
  function overviewSystemPrompt(level) {
    ...
    return (
      '你是我的学习助手。我会给你一段带时间戳的视频字幕(可能是中文或英文),' +
      '请输出一份中文结构化概览,要求:\n' +
      '1) 概要(summary):以第三人称口吻连贯介绍这个视频讲了什么(3-5 句),' +
      '像向别人转述而非摘抄;关键信息处内嵌时间戳锚点 [M:SS](取自字幕行首),让读者能跳回对应位置\n' +
      `2) 章节:${guide};章节必须覆盖整个视频从头到尾,` +
      '最后一章必须覆盖到视频后段,不要只集中在开头\n' +
      '3) 时间戳必须取自字幕行首的 [M:SS],不得编造,不得超过视频时长\n' +
      '只输出 JSON(不要 markdown 代码围栏),格式:\n' +
      '{"summary":"这个视频……[0:45]……","chapters":[' +
      '{"title":"章节标题","timestamp":"0:00","timestampSeconds":0,"summary":"本节内容"}]}'
    );
  }
   ...
   * @returns {Promise<object>} { summary, chapters, level, v }
   */
  async function generateOverview(videoKey, ai, force, level) {
    ...
    if (!force) {
      ...
      // 结构版本不符(v1 关键引述时代)或粒度设置变了:缓存失效,按新结构重建
      if (cached[key] && cached[key].v === OV_SCHEMA_V &&
          (cached[key].level || 'normal') === lv) return cached[key];
    }
    ...
    if (chunks.length === 1) {
      ...
      const userContent =
        ...
        `字幕:\n${buildTranscript(doc)}`;
      overview = await aiChatJson(ai, overviewSystemPrompt(lv), userContent, 'AI 概览返回的 JSON 解析失败,请重试');
    } else {
      // 长视频:分段独立 AI + 末轮合并
      overview = await generateChunkedOverview(doc, cues, chunks, ai, lv);
    }

    if (!Array.isArray(overview.chapters)) overview.chapters = [];
    overview.summary = typeof overview.summary === 'string' ? overview.summary : '';
    overview.v = OV_SCHEMA_V;
    overview.level = lv; // 记录粒度,设置变化时缓存自动失效
    overview.chunkCount = chunks.length; // 调试/展示用
    await chrome.storage.local.set({ [key]: overview });
    return overview;
  }

  /** AI 返回 JSON 的统一封装:剥围栏 + 解析,失败抛带原因的错 */
  async function aiChatJson(ai, system, userContent, parseFailMsg) {
    let content = await aiChat(ai, system, userContent);
    content = content.replace(/^\s*```(?:json)?\s*/i, '').replace(/\s*```\s*$/, '');
    try {
      return JSON.parse(content);
    } catch (e) {
      throw new Error(parseFailMsg || 'AI 返回内容解析失败');
    }
  }

  /**
   * 长视频概览生成:分段独立 AI + 末轮合并
   * 输入:
   *   doc        - VdcCache 字幕文档
   *   cues       - doc.cues
   *   chunks     - chunkByCharBudget 结果
   *   ai         - AI provider 配置
   *   level      - 粒度
   * 流程:
   *   1) 对每个 chunk 调一次 AI,产出局部 JSON { chunkSummary, chapters }
   *   2) 把所有 chunk 结果喂给合并 prompt,产出全局 JSON { summary, chapters }
   * 失败兜底:
   *   - 某段 AI 失败时,该段回退为"无章节",靠合并 prompt 用段交界时间戳补齐
   *   - 合并 AI 失败时,把所有局部 chapters 拼起来 + 用各 chunkSummary 拼 summary
   */
  async function generateChunkedOverview(doc, cues, chunks, ai, level) {
    const duration = cues[cues.length - 1].end;
    const chunkResults = [];
    for (let i = 0; i < chunks.length; i++) {
      const ch = chunks[i];
      const meta = { idx: i + 1, total: chunks.length, startSec: ch.startSec, endSec: ch.endSec };
      const userContent =
        `视频标题:${doc.title || '未知'}\n` +
        `视频时长:${fmtTime(duration)}(${Math.floor(duration)} 秒)\n` +
        `本段时间戳范围:${fmtTime(ch.startSec)} → ${fmtTime(ch.endSec)},不要使用本段范围外的时间戳\n\n` +
        `本段字幕:\n${ch.lines.join('\n')}`;
      let partial;
      try {
        partial = await aiChatJson(
          ai,
          chunkSystemPrompt(level, meta),
          userContent,
          `第 ${meta.idx}/${meta.total} 段局部概览 JSON 解析失败,请重试`
        );
      } catch (e) {
        console.warn('[transnotes] 段概览失败,降级为空:', meta.idx, (e && e.message) || e);
        partial = { chunkSummary: '(本段生成失败)', chapters: [] };
      }
      // 兜底:章节时间戳超本段范围或为空 → 用本段首尾补 2 个空章节,确保末轮合并有空锚点
      if (!Array.isArray(partial.chapters) || partial.chapters.length === 0) {
        partial.chapters = [{
          title: `段 ${meta.idx}`,
          timestamp: fmtTime(ch.startSec),
          timestampSeconds: Math.floor(ch.startSec),
          summary: partial.chunkSummary || '(无摘要)',
        }];
      }
      chunkResults.push({ meta, partial });
    }
    ...
    const mergeInput =
      `视频标题:${doc.title || '未知'}\n` +
      `视频时长:${fmtTime(duration)}(${Math.floor(duration)} 秒),不要使用超过此时长的时间戳\n\n` +
      `各段局部概览(共 ${chunkResults.length} 段,按时间顺序):\n` +
      chunkResults.map(({ meta, partial }) =>
        `--- 段 ${meta.idx}/${meta.total} (${fmtTime(meta.startSec)} → ${fmtTime(meta.endSec)}) ---\n` +
        `本段摘要:${partial.chunkSummary || '(无)'}\n` +
        `本段章节:\n` +
        (Array.isArray(partial.chapters) ? partial.chapters.map(c =>
          `  - ${c.timestamp || ''} ${c.title || ''} :: ${c.summary || ''}`
        ).join('\n') : '(无)')
      ).join('\n\n');

    let overview;
    try {
      overview = await aiChatJson(
        ai,
        mergeSystemPrompt(level),
        mergeInput,
        '合并概览 JSON 解析失败,请重试'
      );
    } catch (e) {
      // 合并 AI 失败 → 本地兜底:把各段章节按时间排序后拼接,summary 取首段
      console.warn('[transnotes] 合并概览失败,降级为本地拼接:', (e && e.message) || e);
      const allChapters = [];
      chunkResults.forEach(({ meta, partial }) => {
        if (Array.isArray(partial.chapters)) allChapters.push(...partial.chapters);
      });
      allChapters.sort((a, b) => (a.timestampSeconds || 0) - (b.timestampSeconds || 0));
      overview = {
        summary: chunkResults.map(c => c.partial.chunkSummary).filter(Boolean).join(' '),
  ...
  async function generateAutoNote(videoKey, ai, opts) {
    ...
    const basePrompt = tplDef.builtin ? tplDef.prompt
      : tplDef.prompt +
    ...
    if (chunks.length === 1) {
      ...
      const userContent =
        ...
        `视频时长:${fmtTime(duration)}(${Math.floor(duration)} 秒),不要使用超过此时长的时间戳\n\n` +
        `字幕:\n${buildTranscript(doc)}`;
      md = await aiChat(ai, basePrompt, userContent, { maxTokens: 8192 });
    } else {
      // 长视频:分段生成局部笔记后拼接
      md = await generateChunkedAutoNote(doc, cues, chunks, ai, basePrompt);
    }
  ...
  /**
   * 长视频自动笔记:分段独立 AI + 拼接
   * 每段 prompt 在 basePrompt 上加"这是第 X/N 段,只写本段"约束
   * 段间用 `---` 分隔 + 段标头(M:SS → M:SS)
   */
  async function generateChunkedAutoNote(doc, cues, chunks, ai, basePrompt) {
    const duration = cues[cues.length - 1].end;
    const partials = [];
    for (let i = 0; i < chunks.length; i++) {
      const ch = chunks[i];
      ...
      const chunkPrompt =
        basePrompt +
        `\n\n本片段是长视频中的第 ${meta.idx}/${meta.total} 段,` +
        `字幕时间 ${fmtTime(ch.startSec)} → ${fmtTime(ch.endSec)}。` +
        '请只针对本段字幕生成笔记,不要输出其他段的章节/总结。';
      const userContent =
        `视频标题:${doc.title || '未知'}\n` +
        `视频时长:${fmtTime(duration)}(${Math.floor(duration)} 秒)\n` +
        `本段时间戳范围:${fmtTime(ch.startSec)} → ${fmtTime(ch.endSec)},不要使用本段范围外的时间戳\n\n` +
      ...
      let partial;
      try {
        partial = await aiChat(ai, chunkPrompt, userContent, { maxTokens: 4096 });
      } catch (e) {
        console.warn('[transnotes] 段笔记失败,降级为占位:', meta.idx, (e && e.message) || e);
        partial = `_本段(${fmtTime(ch.startSec)} → ${fmtTime(ch.endSec)})生成失败,可手动补全_\n`;
      }
      partials.push({ meta, md: partial });
    }
    // 拼接:每段前加段标头(便于阅读和侧边栏定位),段间分隔
    const parts = [];
    partials.forEach(({ meta, md }) => {
      parts.push(`<!-- 段 ${meta.idx}/${meta.total} (${fmtTime(meta.startSec)} → ${fmtTime(meta.endSec)}) -->`);
      parts.push(md.trim());
      parts.push('');
    });
    return parts.join('\n---\n\n');
    ...
    if (sections.qa) {
      ...
      if (qaList.length) {
        ...
        for (const qa of qaList) {
          ...
          if (qa.shot) out.push(`  ![](attachments/${qa.shot}.jpg)`); // 提问时的视频画面
          out.push('');
        }
      }
    }
