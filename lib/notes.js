...
(function () {
  ...
  async function generateAutoNote(videoKey, ai, opts) {
    ...
    let md = await aiChat(ai, NOTE_TEMPLATES[tpl].prompt, userContent, { maxTokens: 8192 });
  ...
  async function buildMarkdown(videoKey, opts) {
    ...
    if (sections.autoNote) {
      ...
      const tplName = autoNote && NOTE_TEMPLATES[autoNote.template]
        ...
        : (NOTE_TEMPLATES[autoNoteTemplate] || NOTE_TEMPLATES[DEFAULT_TEMPLATE]).name;
  ...
  async function generateDraft(videoKey, ai, opts) {
    ...
    if (wantAutoNote) {
      ...
      // last 指针是后加的,存量数据没有,故再兜底遍历全部已知模板(仅 4 次本地读)
      const lastKey = `${ANOTE_PREFIX}last:${videoKey}`;
      const lastTpl = (await chrome.storage.local.get(lastKey))[lastKey];
      const candidates = [...new Set(
        [lastTpl, ...Object.keys(NOTE_TEMPLATES)].filter((t) => t && t !== autoNoteTemplate)
      )];
