...
(function () {
   ...
   * @returns {Promise<object>} 问答记录 { id, ts, question, answer, createdAt }
   */
  async function ask(videoKey, question, t, ai) {
    const q = String(question || '').trim();
    if (!q) throw new Error('问题不能为空');
    if (!ai || !ai.apiKey) throw new Error('AI API Key 未配置,请在设置页填写文本模型(助教与翻译共用)');
    const userContent = await buildContext(videoKey, typeof t === 'number' ? t : 0, q);
    const answer = await VdcNotes.aiChat(ai, SYSTEM_PROMPT, userContent, { maxTokens: 4096 });
    const qa = {
