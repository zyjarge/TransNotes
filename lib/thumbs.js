...
(function () {
  ...
  function parsePvdata(data, durationSec) {
    ...
    const spriteTotal = perSprite * data.image.length;
    // 实际帧数:B 站 pvdata 是 { "0": sec, "1": sec, ... } 形式, maxKey+1 = 真实帧数;
    // 若 pvdata 缺失或为空, fallback 用 spriteTotal (雪碧图理论容量).
    let actualTotal = spriteTotal;
    if (data.pvdata && typeof data.pvdata === 'object') {
      const keys = Object.keys(data.pvdata);
      if (keys.length) {
        let max = -1;
        for (const k of keys) {
          const n = +k;
        ...
        if (max >= 0) actualTotal = max + 1;
      }
    }
    const total = actualTotal;  // 后续 frameAt 用此 total 算 idx, 防止越界指向空白帧
    const frameAt = (t) => {
      let idx;
      if (Array.isArray(data.f_times) && data.f_times.length) {
        idx = nearestIndex(data.f_times, t);
      } else {
        // 优先用实际视频时长算 idx, fallback 到 total*perSprite/columns(保守估算)
