...
(function () {
  ...
  function createChunkHandler(hooks) {
    ...
    return async function handleChunk(msg) {
      ...
      console.log('[transnotes] 合并音频切分完成:新增', added, '句 (已缓存', cache.size, '句)');
  ...
  /**
   * 把整页截图裁剪到指定元素矩形(默认视频画面),并按 maxWidth 等比缩小。
   * 截图 dataURL 来自扩展自己的 captureVisibleTab,canvas 无跨域污染;
   * 元素不可见/不在视口时退回整页截图。助教带图提问等场景使用。
   */
  async function cropToElement(dataUrl, el, maxWidth = 1280) {
    ...
    if (el && el.getBoundingClientRect) {
      ...
      sx = Math.max(0, r.left * scaleX);
      sy = Math.max(0, r.top * scaleY);
      sw = Math.min(img.naturalWidth - sx, r.width * scaleX);
      sh = Math.min(img.naturalHeight - sy, r.height * scaleY);
      if (sw < 10 || sh < 10) { sx = 0; sy = 0; sw = img.naturalWidth; sh = img.naturalHeight; }
    }
