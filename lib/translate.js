...
(function () {
   ...
   */
  async function requestTranslation(texts, { baseUrl, apiKey, model, disableThinking }) {
   ...
   * @param {object} options { baseUrl, apiKey, model, disableThinking }
   * @returns {Promise<string[]>} 中文译文数组(与输入一一对应)
   */
  async function translateBatch(texts, { baseUrl, apiKey, model, disableThinking }) {
    if (!apiKey) throw new Error('翻译 API Key 未配置,请在设置页填写');
    if (!baseUrl) throw new Error('翻译 API 地址未配置,请在设置页填写');
    if (!texts || texts.length === 0) return [];
    return translateChunk(texts, { baseUrl, apiKey, model, disableThinking });
  }
